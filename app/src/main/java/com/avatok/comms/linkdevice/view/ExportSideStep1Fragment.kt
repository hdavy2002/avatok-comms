 ...
 */
package com.avatok.comms.linkdevice.view
...
import com.avatok.comms.databinding.FragmentExportSideStep1Binding
...
import com.avatok.comms.linkdevice.viewmodel.ExportSideInputError
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.code.addTextChangedListener(object : TextWatcher {
            ...
            override fun afterTextChanged(currentText: Editable) {
                ...
                if (!currentTextStr.startsWith(SCHEME, ignoreCase = true)) {
                    ...
                    val interestingText =
                        if (currentTextStr.length > SCHEME.length)
                            currentTextStr.substring(SCHEME.length + 1)
                        else ""
                    val finalText = SCHEME + interestingText
                    ...
                    val interestingText = currentTextStr.substring(SCHEME.length)
                    if (interestingText.startsWith(SCHEME, ignoreCase = true)) {
                        ...
                        val reallyInterestingText = interestingText.substring(SCHEME.length)
                        val finalText = SCHEME + reallyInterestingText
                        binding.code.setText(finalText)
                        binding.code.setSelection(finalText.length)
                    }
                }
